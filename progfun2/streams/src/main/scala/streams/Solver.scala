...
trait Solver extends GameDef {
  ...
  /**
    ...
    */
  def done(b: Block): Boolean = b.isLegal && b.isStanding && (b.b1 == goal || b.b2 == goal)
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, history) #:: tail =>
      val neighbor = newNeighborsOnly(neighborsWithHistory(block, history), explored)
      (block, history) #:: from(tail ++ neighbor, explored + block)
    case _ => Stream.empty
  }
